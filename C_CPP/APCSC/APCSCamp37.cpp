...
int n,m,k;
//vector<vector<stack<int> > > v(100001,vector<stack<int> >(100001));
map<pair<int,int>,stack<int> > mm;
inline void solve(){
    int T;
    cin >> n >> m >> T;
    while(T--){
        ...
        if(x==1){
            ...
            int i,j;
            cin >> i >> j;
            if(mm[{i,j}].empty()) cout << "empty\n";
            else cout << mm[{i,j}].top()<<endl;
        }
        else if(x==3){
            int i,j;
            cin >> i >> j;
            if(!mm[{i,j}].empty()) mm[{i,j}].pop();
        }
    }
