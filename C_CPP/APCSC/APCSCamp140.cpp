...
int dp[200005];
int n;
int LIS(pair<int,int>  *v){ //Longest Increasing Subsequence Length
	fill(dp,dp+n,MAX);
    for(int i=0;i<n;i++){
        auto j = upper_bound(dp,dp+n,v[i].second) - dp;
        dp[j] = v[i].second;
    }
    return lower_bound(dp,dp+n,MAX) - dp;
}
inline int nextint(){
...
inline void solve(){
	...
	cin >> n;
	fr(0,n) cin >> p[i].first >> p[i].second;
	sort(p,p+n);
