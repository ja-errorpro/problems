...
int arr[MAXN];
int sav[MAXN];
int mergecnt(int l,int r){
	if(l+1==r) return 0;
	int cnt=0;
	int mid = l+((r-l)>>1);
	cnt+=mergecnt(l,mid);
	cnt+=mergecnt(mid,r);
	int ss=l,ee=mid,kk=l,re=mid-1;
	fr(i,l,mid){
		while(re+1<r&&arr[i]>arr[re+1]) re++,ans+=arr[i]+arr[re+1];
		cnt+=re-mid+1;
	}
	while(ss<mid||ee<r){
		if(ss<mid&&(ee>=r||arr[ss]<arr[ee])){
			sav[kk++] = arr[ss++];
			ans+=arr[ss-1];
		}
		else{
