...
inline void solve(){
	...
	cin >> s;
	if(*lower_bound(word.begin(),word.end(),s,cmp)!=s){
		cout << 0 << endl;
	}
	else{
		cout << lower_bound(word.begin(),word.end(),s,cmp) - word.begin()+1 << endl;
	}
...
signed main(){
	...
	for(int i=0;i<4;i++){
		for(int j=l;j<r;j++){
			for(char k=word[j][i]+1;k<='z';k++){
				word.push_back(word[j]+k);
			}
		}
		l=r;
		r=word.size();
