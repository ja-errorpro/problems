...
inline void solve(){
	...
	fr(0,q){
		int k;
		cin >> k;
		vector<pair<int,int> >::iterator u = upper_bound(a.begin(),a.end(),make_pair(k,n));
		vector<pair<int,int> >::iterator l = lower_bound(a.begin(),a.end(),make_pair(k,1));
		int c = u - l;
		//cout << c << endl;
		if(c){
			cout << a[l-a.begin()].second << ' ' << c << endl;
		}
		else cout << "-1 0" << endl;
