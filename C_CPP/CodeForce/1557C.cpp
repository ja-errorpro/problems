...
inline void solve(){
	...
	a[0]=1;
	if(n%2){
		frc(1,k) a[i]=(a[i-1]%MAX * (pows(2,n-1,MAX)+1)%MAX)%MAX;
	}
	else{
		frc(1,k){
		
			ll offset = pows(2,k-1,MAX);
			offset=pows(offset,n,MAX);
			a[i]=offset+(a[i-1]%MAX * (pows(2,n-1,MAX)-1)%MAX)%MAX;
		}
	}
