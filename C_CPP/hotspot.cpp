...
inline void solve(){
	...
	double dy = (r*sqrt(3.0)/2.0);
	double dx = 1.5*r;//sqrt(r*r-r*r*3/4)
	//cout << dx <<' '<<dy<<endl;
	int movx=0,movy=0;
	while(x+dx<=1000.0){
		x+=dx;
	...
	//cout << movx << ' ' << movy << endl;
	// let x and y to left down point.
	int rcnt=0;
	...
	if((movx*movy)%2 && (ccnt*rcnt)%2){
		cout << (ccnt*rcnt)/2+1 << endl;
	}
	else{
		cout  <<((ccnt)*(rcnt))/2  << endl;
	}
