...
inline void wwriteint(long long d){
    if(d==0){
        putchar_unlocked(48);
        return;
    }
...
int m,n;
string decode(string t,string e){
    string ret,a,b,c="",s="";
    deque<char> dq;
    for(int i=n-1;i>=0;i--){
        if(e[i]=='1'){
            dq.push_back(t[i]);
        }else if(e[i]=='0') dq.push_front(t[i]);
    }
    while(!dq.empty()) s+=dq.front(),dq.pop_front();
    //cout << s << endl;
    int one_cnt=0;
    for(int i=0;i<n;i++) one_cnt += (e[i]=='1');
    ret = s;
    if(one_cnt&1){
        if(n&1){
            a = s.substr(0,n/2);
...
inline void solve(){
    ...
    for(int i=m-1;i>=0;i--) T = decode(T,E[i]);
	cout << T;
