...
bool check(pii A,pii B,pii C){
    int f = (B.fi-A.fi)*(C.se-B.se) + (B.se - C.se) * (C.fi-B.fi);
    return (f>0);
}
inline void solve(){
    ...
    frc(i,1,4){
       if(i==1){
            firs = check(pos[4],pos[i],pos[i+1]);
            cout << check(pos[4],pos[i],pos[i+1]) << endl;
            continue;
       }
        else if(i==4&&firs!=check(pos[i-1],pos[i],pos[1])){
            ok = false;
        }
        else if(firs!=check(pos[i-1],pos[i],pos[(i+1)])){
            ok = false;
        }
